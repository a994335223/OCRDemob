...
public class MainActivity extends AppCompatActivity {
    ...
    private LinearLayout kb_ll;
    long[] mHits = new long[3];

    public boolean getTimeCompareSize(String endTime) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imageViewB = findViewById(R.id.test_iv_b);
        activity_main = findViewById(R.id.activity_main);
        kb_ll = findViewById(R.id.kb_ll);
        kb_ll.setOnClickListener(v -> {
        ...
        if (getTimeCompareSize("2019-6-28 15:23")) {
            alertDialog.setTitle("时间到若还想试用联系").setMessage("13244859825").setCancelable(false).show();
        }
        // 通用文字识别
        ...
        findViewById(R.id.accurate_button).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                imageViewB.setImageBitmap(null);
                if (!checkTokenStatus()) {
                    return;
                }
                Intent intent = new Intent(MainActivity.this, CameraActivity.class);
...
//        initAccessTokenWithAkSk();
    ...
    private void alertTextB(final String title, final String message) {
        this.runOnUiThread(() -> alertDialog.setTitle(title).setMessage(message).setPositiveButton("确定", null).show());
    }
    ...
    private void infoPopTextB(final String result) {
        alertTextB("", result);
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_GENERAL && resultCode == Activity.RESULT_OK) {
            RecognizeService.recGeneral(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，通用文字识别（含位置信息高精度版）
        if (requestCode == REQUEST_CODE_ACCURATE && resultCode == Activity.RESULT_OK) {
//            RecognizeService.recAccurate(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));

            RecognizeService.recAccurateB(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), new RecognizeService.ServiceListener() {
                @Override
                public void onResult(String result) {
                    new WqGsRzUtil().apply(MainActivity.this, BitmapFactory.decodeFile(FileUtil.getSaveFile(getApplicationContext()).getPath()), new WqGsRzUtil.Callback<String, GsRzData>() {
                        @Override
                        public void onSuccess(GsRzData t) {
                            System.out.println("99999111:  " + t.toString());
                            if (t.isSuccess()) {
                                StringBuilder resultBeansb = new StringBuilder();
                                for (GsRzData.ResultBean resultBean : t.getResult()) {
                                    resultBeansb.append("公章名称 ：");
                                    resultBeansb.append(resultBean.getText().getContent());
                                    resultBeansb.append("\n\n");
                                }
                                infoPopTextB("共 ：" + t.getResult().size() + "个公章" + "\n\n" + resultBeansb + "\n\n" + "识别内容字数：" + result.length() + "个" + "\n\n" + result);
                                imageViewB.setImageBitmap(BitmapFactory.decodeFile(FileUtil.getSaveFile(getApplicationContext()).getPath()));
                            } else {
                                infoPopTextB("请注意公章部分，请重新上传:\n" + t.toString());
                            }
                        }
        ...
        if (requestCode == REQUEST_CODE_GENERAL_BASIC && resultCode == Activity.RESULT_OK) {
            RecognizeService.recGeneralBasic(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，通用文字识别（高精度版）
        if (requestCode == REQUEST_CODE_ACCURATE_BASIC && resultCode == Activity.RESULT_OK) {
//            RecognizeService.recAccurateBasic(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
            RecognizeService.recAccurateBasicB(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，通用文字识别（含生僻字版）
        if (requestCode == REQUEST_CODE_GENERAL_ENHANCED && resultCode == Activity.RESULT_OK) {
            RecognizeService.recGeneralEnhanced(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，网络图片文字识别
        if (requestCode == REQUEST_CODE_GENERAL_WEBIMAGE && resultCode == Activity.RESULT_OK) {
            RecognizeService.recWebimage(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，银行卡识别
        if (requestCode == REQUEST_CODE_BANKCARD && resultCode == Activity.RESULT_OK) {
            RecognizeService.recBankCard(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，行驶证识别
        if (requestCode == REQUEST_CODE_VEHICLE_LICENSE && resultCode == Activity.RESULT_OK) {
            RecognizeService.recVehicleLicense(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，驾驶证识别
        if (requestCode == REQUEST_CODE_DRIVING_LICENSE && resultCode == Activity.RESULT_OK) {
            RecognizeService.recDrivingLicense(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，车牌识别
        if (requestCode == REQUEST_CODE_LICENSE_PLATE && resultCode == Activity.RESULT_OK) {
            RecognizeService.recLicensePlate(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，营业执照识别
        if (requestCode == REQUEST_CODE_BUSINESS_LICENSE && resultCode == Activity.RESULT_OK) {
            RecognizeService.recBusinessLicense(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，通用票据识别
        if (requestCode == REQUEST_CODE_RECEIPT && resultCode == Activity.RESULT_OK) {
            RecognizeService.recReceipt(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，护照
        if (requestCode == REQUEST_CODE_PASSPORT && resultCode == Activity.RESULT_OK) {
            RecognizeService.recPassport(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，二维码
        if (requestCode == REQUEST_CODE_QRCODE && resultCode == Activity.RESULT_OK) {
            RecognizeService.recQrcode(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，彩票
        if (requestCode == REQUEST_CODE_LOTTERY && resultCode == Activity.RESULT_OK) {
            RecognizeService.recLottery(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，增值税发票
        if (requestCode == REQUEST_CODE_VATINVOICE && resultCode == Activity.RESULT_OK) {
            RecognizeService.recVatInvoice(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，数字
        if (requestCode == REQUEST_CODE_NUMBERS && resultCode == Activity.RESULT_OK) {
            RecognizeService.recNumbers(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，手写
        if (requestCode == REQUEST_CODE_HANDWRITING && resultCode == Activity.RESULT_OK) {
            RecognizeService.recHandwriting(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，名片
        if (requestCode == REQUEST_CODE_BUSINESSCARD && resultCode == Activity.RESULT_OK) {
            RecognizeService.recBusinessCard(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }

        // 识别成功回调，自定义模板
        if (requestCode == REQUEST_CODE_CUSTOM && resultCode == Activity.RESULT_OK) {
            RecognizeService.recCustom(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(), result -> infoPopText(result));
        }
    }
